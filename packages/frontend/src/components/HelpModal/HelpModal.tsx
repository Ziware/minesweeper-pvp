import React from 'react';
import styles from './HelpModal.module.css';

interface HelpModalProps {
  onClose: () => void;
}

export function HelpModal({ onClose }: HelpModalProps) {
  return (
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        <button className={styles.closeBtn} onClick={onClose}>✕</button>
        <h2 className={styles.title}>💣 Правила игры</h2>

        <div className={styles.content}>
          <section>
            <h3>🎯 Цель</h3>
            <p>
              Главная цель — захватить штаб противника: зону 1×2 клетки по центру
              края доски. Захват любой клетки штаба сразу приносит победу.
              Если за 15 ходов каждого игрока штаб не взят — побеждает тот,
              кто контролирует больше поля.
            </p>
          </section>

          <section>
            <h3>🗺️ Поле</h3>
            <p>
              Поле 10×10. Красный владеет верхней половиной, синий — нижней.
              Штаб красного находится по центру верхнего края, штаб синего — по
              центру нижнего края. Штаб нельзя заминировать. В начале каждый
              расставляет по <strong>7 мин</strong> на своей половине. Мины противника скрыты.
            </p>
          </section>

          <section>
            <h3>🔄 Ход игрока (3 фазы)</h3>

            <div className={styles.phase}>
              <div className={styles.phaseTitle}>Фаза 1 — Выбор зоны</div>
              <p>
                Кликните на любую клетку поля — вокруг неё появится зона
                <strong> 3×3</strong> (жёлтая рамка) и зона <strong>5×5</strong> (синяя рамка).
                В зоне 3×3 должна быть хотя бы одна <em>доступная</em> союзная клетка:
                такая клетка соединена с вашим штабом непрерывным путём по соседним
                союзным клеткам через общие стороны.
                На своих клетках в зоне 3×3 появятся цифры — сколько мин противника
                находится рядом с каждой клеткой.
              </p>
            </div>

            <div className={styles.phase}>
              <div className={styles.phaseTitle}>Фаза 2 — Захват клеток</div>
              <p>
                В пределах зоны <strong>5×5</strong> можно захватывать клетки противника
                кликом по ним. Захват возможен только если захватываемая клетка
                соседствует по общей стороне хотя бы с одной <em>доступной</em> клеткой
                вашей территории — диагонали не считаются.
              </p>
              <p>
                Если в клетке <strong>есть мина</strong> — вы теряете жизнь, мина исчезает,
                клетка остаётся за противником, ход переходит к фазе 3.
              </p>
              <p>
                Обычно один раз за ход можно <strong>разминировать</strong> доступную для захвата клетку
                (<kbd>Ctrl+Click</kbd>). Если зона 5×5 содержит хотя бы одну клетку вашего штаба,
                зона считается защитной и даёт <strong>2 разминирования</strong> в этом ходу.
                Если мина есть — она убирается, клетка переходит к вам, ход продолжается.
                Если мины нет — клетка переходит к вам, фаза 2 завершается и начинается фаза 3.
              </p>
              <p>
                Можно добровольно завершить фазу кнопкой <em>«Завершить захват»</em>.
              </p>
            </div>

            <div className={styles.phase}>
              <div className={styles.phaseTitle}>Фаза 3 — Расстановка мин</div>
              <p>
                Можно поставить от <strong>0 до 3 мин</strong> на свободные доступные клетки
                своей территории. Завершить расстановку можно вручную; поражения
                из-за нехватки места для мин больше нет.
              </p>
            </div>
          </section>

          <section>
            <h3>🏁 Завершение игры</h3>
            <ul>
              <li>Мгновенная победа за захват любой клетки штаба противника.</li>
              <li>Победа противника, если потеряны все <strong>3 жизни</strong> (взрывы на минах).</li>
              <li>После 15 ходов красного и 15 ходов синего побеждает игрок с большей территорией.</li>
            </ul>
          </section>

          <section>
            <h3>🚩 Метки</h3>
            <p>
              <strong>ПКМ</strong> по клетке противника циклически меняет метку:
              нет → 🚩 флаг → ❓ вопрос → нет.
              Флаги помогают отмечать где, по-вашему, стоят мины.
              На свои клетки метки ставить нельзя.
            </p>
          </section>

          <section>
            <h3>🔢 Цифры</h3>
            <p>
              Цифра на клетке показывает сколько мин противника находится
              в 8 соседних клетках. <strong>0</strong> — мин рядом нет.
              Мины на своих клетках в зоне 3×3 скрыты — видны только цифры.
              Захват при этом выполняется только через 4 клетки по общей стороне.
            </p>
          </section>
        </div>
      </div>
    </div>
  );
}
